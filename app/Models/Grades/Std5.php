...
//            SELECT student_id,AVG(mathematics + reading_and_writing + english + art_and_craft)/4 AS average FROM nursery GROUP BY student_id ORDER BY average DESC
    ...
    {
        return Std5::where('term', $term)->avg('Mathematics');
        //SELECT AVG(Mathematics) FROM `standard_5` where term = 1
    }

    public static function avgEng($term)
    {
        return Std5::where('term', $term)->avg('English');
        //SELECT AVG(English) FROM `standard_5' where term = 1
    }

    public static function avgKis($term)
    {
        return Std5::where('term', $term)->avg('Kiswahili');
        //SELECT AVG(Kiswahili) FROM `standard_5` where term = 1
    }

    public static function avgScTec($term)
    {
        return Std5::where('term', $term)->avg('Science_and_Technology');
        //SELECT AVG(Science_and_Technology) FROM `standard_5` where term = 1
    }

    public static function avgCivMor($term)
    {
        return Std5::where('term', $term)->avg('Civics_and_Moral');
        //SELECT AVG(Civics_and_Moral) FROM `standard_5` where term = 1
    }

    public static function avgSs($term)
    {
        return Std5::where('term', $term)->avg('Social_Studies');
        //SELECT AVG(Social_Studies) FROM `standard_5` where term = 1
    }

    public static function avgVS($term)
    {
        return Std5::where('term', $term)->avg('Vocational_Skills');
        //SELECT AVG(Vocational_Skills) FROM `standard_5` where term = 1
    }
    ...
    {
        return Std5::where('term', $term)->max('Science_and_Technology');
        //SELECT MAX(Science_and_Technology) FROM `standard_5 where term = 1
    }

    public static function maxCivMor($term)
    {
        return Std5::where('term', $term)->max('Civics_and_Moral');
        //SELECT MAX(Civics_and_Moral) FROM `standard_5` where term = 1
    }

    public static function maxSs($term)
    {
        return Std5::where('term', $term)->max('Social_Studies');
        //SELECT MAX(Social_Studies) FROM `standard_5` where term = 1
    }

    public static function maxVS($term)
    {
        return Std5::where('term', $term)->max('Vocational_Skills');
        //SELECT MAX(Vocational_Skills) FROM `standard_5` where term = 1
    }
