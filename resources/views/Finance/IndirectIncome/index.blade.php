...
@section('content')
                ...
                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap"
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td class="desc_name">{{$item->event}}</td>
                            <td>{{$item->value}}</td>
                            <td>{{$item->date}}</td>
                            <td style="width: 10%">
                                <a href="{{url('IndirectIncome/edit/'.$item->id)}}" class="edit-btn"> Edit</a> |
                                <a href="{{url('IndirectIncome/delete/'.$item->id)}}" class="delete-btn"> Delete</a>
    ...
    <div class="modal fade" role="dialog" id="create" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{url('IndirectIncome/store')}}" method="post">
                    @csrf
                    <div class="modal-header modal-header-color">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                    aria-hidden="true">×</span>
                        </button>
                        <h4 class="modal-title"><strong>Add Income</strong></h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group row">
                            <div class="col-lg-4 col-md-4">
                                <label for="event" class="control-label">Event</label>
                                <input class="form-control input-sm" id="event" name="event"
                                       type="text" required>
                            </div>
                            <div class="col-lg-4 col-md-4">
    ...
    <div class="modal fade" role="dialog" id="edit" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{url('IndirectIncome/update')}}" method="post">
                    @csrf
                    <div class="modal-header modal-header-color">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                    aria-hidden="true">×</span>
                        </button>
                        <h4 class="modal-title"><strong> Edit Income</strong></h4>
                    </div>
                    <div class="modal-body">
