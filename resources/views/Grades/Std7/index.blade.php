...
@section('title','Standard 7 Grades')
...
@section('content')
                ...
                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap"
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td>
                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole('academic_teacher'))
                                <a href="{{url('Std7/edit/'.$item->id)}}" class="edit-btn"> Edit</a> |
                                <a href="{{url('Std7/delete/'.$item->id)}}" class="delete-btn"> Delete</a>
                                @endif
                                <a href="{{url('Std7/show/'.$item->id)}}" class="btn-white"> View</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    <div class="modal fade" role="dialog" id="create" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <form action="{{url('Std7/store')}}" method="post">
                    @csrf
    ...
    <div class="modal fade" role="dialog" id="edit" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <form action="{{url('Std7/update')}}" method="post">
                    @csrf
                    <div class="modal-header modal-header-color">
                        <button type="button"  class="close" data-dismiss="modal" aria-label="Close"><span
                                    aria-hidden="true">×</span>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header modal-header-color">
                    ...
                    <h4 class="modal-title"><strong>Standard 7 Marks</strong></h4>
                </div>
                <div class="modal-body">
    ...
    <script>
        ...
        $("#edit").on('shown.bs.modal',function () {
            ...
            $(".lesson").change(function () {
                ...
                $.ajax({
                    url: '{{url('/grades/ajax/7')}}',
                    type: "GET",
                    data: {d_lesson: lesson, d_Std7: Std7_id},
                    dataType: "json",
